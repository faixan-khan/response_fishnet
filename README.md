...
Clarification: Thank you for the valuable feedback. We agree that the zero-shot nature of the task introduces significant challenges, even for higher-level taxonomic recognition such as families. To empirically support this, we trained a supervised ConvNeXt model (pretrained on ImageNet) on our dataset for family-level classification. This model achieves approximately 80% accuracy, demonstrating that substantially higher performance is indeed attainable under supervised conditions. This suggests that the lower zero-shot performance of multimodal language-vision models (MLLMs) may be attributed to a lack of domain-specific (e.g., marine-related) pretraining data. This empirical result helps contextualize the difficulty of our zero-shot benchmark and highlights the gap between general-purpose pretraining and fine-grained biological recognition tasks.
