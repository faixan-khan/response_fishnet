...
**Q3** *The relatively lower accuracy at the Family level can largely be attributed to the significantly broader taxonomic coverage in our dataset. As shown in Table 1 of the main paper, most prior works focus on a limited set of species and families, which naturally leads to higher accuracy. In contrast, our benchmark includes over 17,000 species spanning 624 families, making the classification task considerably more challenging. Moreover, our evaluation is conducted in a zero-shot setting, which further raises the difficulty. Despite this, achieving 46.2%–53.6% accuracy at the Family level is substantially better than random chance (which would be below 0.2%), representing over 300× improvement over random guessing. We believe these results are reasonable given the scale and open-world nature of the task.*

*While the authors' response is plausible, it is mostly speculative because it lacks empirical support within the paper. Wordings like "can largely be attributed to" and "we believe these results are reasonable" don’t substitute for analysis. These are interpretations, not conclusions drawn from controlled experiments. Saying 46.2-53.6% is "300× better than random" is not meaningful in this context. In classification tasks with hundreds of classes, this comparison often inflates the perceived performance. I believe that additional experiments, for example, evaluating the performance of the same model in a few-shot or supervised setting, would have been necessary to make such a claim.*

Clarification: Thank you for the valuable feedback. We agree that the zero-shot nature of the task introduces significant challenges, even for higher-level taxonomic recognition such as families. To empirically support this, we trained supervised models (pretrained on ImageNet) on our dataset for family-level classification. This best model achieves approximately ~80% accuracy, demonstrating that substantially higher performance is indeed attainable under supervised conditions. This suggests that the lower zero-shot performance of multimodal language-vision models (MLLMs) may be attributed to a lack of domain-specific (e.g., marine-related) pretraining data. Please note that the models are evaluated under supervised closed-label setting while MLLMs were evaluated in a zero-shot open-vocabulary setting.
...
**Q4** *All 50 passed the check. However, it is important to clarify that these summaries are designed to mimic the style and content of FishBase descriptions rather than serve as formal taxonomic entries. As such, the threshold for accuracy is lower, focusing more on capturing key visual cues relevant for species recognition.*

*I don’t believe the response fully addresses the concern. The authors note that the descriptions are meant to mimic FishBase entries and that the threshold for accuracy is lower, but this threshold is not clearly defined. What criteria were used for evaluation? Also, given that LLM-generated text is often speculative, the claim that all 50 sampled descriptions passed expert evaluation seems implausible without more detail. It would strengthen the paper to include examples, describe what constituted a “pass”, and ideally provide some breakdown of common issues (if any). Evaluating only 50 samples (~0.36% of the generated set) seems insufficient to confidently assert quality across the full dataset.*

Clarification: The descriptions were not directly generated by GPT-4o. Instead, species-specific information was first collected from reliable sources such as FishBase and other authoritative references. GPT-4o was then used to extract visually discriminative attributes from these expert-verified texts to aid in classification. This approach ensures that the content is grounded in domain expertise, with GPT-4o's role focused on structuring the information to highlight visual cues relevant for downstream tasks. We thanks the reviewer for the feedback and will include detailed discussion about the quality control for descriptions, add evaluate a larger subset by the domain experts.
...
**Q8** *We have one description for each species. The descriptions are provided for over 35,000 species that represent all fish species listed on FishBase at the time the work was conducted.*

**Lines 64–65 state: "FishNet++ is a large-scale dataset comprising 99,556 images across 17,393 fish species, all mapped to current taxonomic standards." Also, Table 1 states 17,393 species and 35,133 descriptions for FishNet++. Something doesn't add up here, and a clarification or correction is required.**

Clarification: FishNet++ includes images corresponding to 17,393 marine species. In total, FishBase catalogs 35,133 species, but for the remaining 17,740 species, no images are available on FishBase or other major marine data sources. However, these species do have entries on FishBase, often accompanied by structural information, textual descriptions, or illustrative drawings. Leveraging this available information, we generate textual descriptions for all species, resulting in a complete set of 35,133 species descriptions.
